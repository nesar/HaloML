...
#            self.allPara = json.load(json_data)
    ...
    def load_data(self): # randomize and split into train and test data
        ...
        allData = allData[shuffleOrder]/(m_particle/1e4)
        allPara1 = allPara1[shuffleOrder]/(1e6*m_particle)
        allPara2 = allPara2[shuffleOrder]/1000.
...
max_features = int(np.max(x_train))+1

print('Building model...')
model = Sequential()
model.add(Embedding(max_features, max_words*2, input_length=max_words))
model.add(Bidirectional(LSTM(max_words*2)))
